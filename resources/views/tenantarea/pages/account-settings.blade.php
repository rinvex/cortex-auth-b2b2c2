        ...
        <div class="row profile">
            ...
            <div class="col-md-9">
                <div class="profile-content">
                    ...
                    {{ Form::model($currentUser, ['url' => route('tenantarea.account.settings.update'), 'id' => 'tenantarea-account-settings-form', 'files' => true]) }}
                                ...
                                <div class="form-group has-feedback{{ $errors->has('profile_picture') ? ' has-error' : '' }}">
                                    ...
                                    @if ($currentUser->exists && $currentUser->getMedia('profile_picture')->count())
                                        ...
                                        <a href="#" data-toggle="modal" data-target="#delete-confirmation"
                                           data-modal-action="{{ route('tenantarea.account.media.destroy', ['member' => $currentUser, 'media' => $currentUser->getFirstMedia('profile_picture')]) }}"
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="form-group has-feedback{{ $errors->has('cover_photo') ? ' has-error' : '' }}">
                                    ...
                                    @if ($currentUser->exists && $currentUser->getMedia('cover_photo')->count())
                                        ...
                                        <a href="#" data-toggle="modal" data-target="#delete-confirmation"
                                           data-modal-action="{{ route('tenantarea.account.media.destroy', ['member' => $currentUser, 'media' => $currentUser->getFirstMedia('cover_photo')]) }}"
                                           data-modal-title="{{ trans('cortex/foundation::messages.delete_confirmation_title') }}"
                                           data-modal-button="<a href='#' class='btn btn-danger' data-form='delete' data-token='{{ csrf_token() }}'><i class='fa fa-trash-o'></i> {{ trans('cortex/foundation::common.delete') }}</a>"
