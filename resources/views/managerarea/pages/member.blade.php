...
{{-- Page Title --}}
@section('title')
    {{ config('app.name') }} » {{ trans('cortex/foundation::common.managerarea') }} » {{ trans('cortex/auth::common.members') }} » {{ $member->exists ? $member->username : trans('cortex/auth::common.create_member') }}
@endsection

@push('inline-scripts')
    {!! JsValidator::formRequest(Cortex\Auth\B2B2C2\Http\Requests\Managerarea\MemberFormRequest::class)->selector("#managerarea-members-create-form, #managerarea-members-{$member->getKey()}-update-form") !!}
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            ...
            <div class="nav-tabs-custom">
                @if($member->exists && $currentUser->can('delete', $member)) <div class="pull-right"><a href="#" data-toggle="modal" data-target="#delete-confirmation" data-modal-action="{{ route('managerarea.members.destroy', ['member' => $member]) }}" data-modal-title="{!! trans('cortex/foundation::messages.delete_confirmation_title') !!}" data-modal-body="{!! trans('cortex/foundation::messages.delete_confirmation_body', ['type' => 'member', 'name' => $member->username]) !!}" title="{{ trans('cortex/foundation::common.delete') }}" class="btn btn-default" style="margin: 4px"><i class="fa fa-trash text-danger"></i></a></div> @endif
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="details-tab">
                        ...
                        @if ($member->exists)
                            {{ Form::model($member, ['url' => route('managerarea.members.update', ['member' => $member]), 'id' => "managerarea-members-{$member->getKey()}-update-form", 'method' => 'put', 'files' => true]) }}
                        @else
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="form-group has-feedback{{ $errors->has('profile_picture') ? ' has-error' : '' }}">
                                        ...
                                        @if ($member->exists && $member->getMedia('profile_picture')->count())
                                            ...
                                            <a href="#" data-toggle="modal" data-target="#delete-confirmation" data-modal-action="{{ route('managerarea.members.media.destroy', ['member' => $member, 'media' => $member->getFirstMedia('profile_picture')]) }}" data-modal-title="{{ trans('cortex/foundation::messages.delete_confirmation_title') }}" data-modal-body="{{ trans('cortex/foundation::messages.delete_confirmation_body', ['type' => 'media', 'name' => $member->getFirstMedia('profile_picture')->file_name]) }}" title="{{ trans('cortex/foundation::common.delete') }}"><i class="fa fa-trash text-danger"></i></a>
                                        @endif
